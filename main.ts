...
function display128 (text: string) {
    matrix.clearMatrix()
    matrix.imageArrayDigit(matrix.matrix_text(rtcpcf85063tp.getTime(rtcpcf85063tp.ePart.ohne)))
    matrix.rasterCircle(64, 64, 63)
    matrix.rasterCircle(64, 96, 31)
    matrix.line(64, 64, 127, 127)
    matrix.writeDigits(0, 0, 4, matrix.matrix_text(text), matrix.eAlign.links, 8, 0, matrix.eTransparent.u)
    ...
    matrix.writeImage(matrix.imageDrehen(matrix.getImage(0), matrix.eZeichenDrehen.rechts), 0, 22, matrix.eTransparent.t, matrix.oled_eFaktor(matrix.eFaktor.f3))
    matrix.writeImage(matrix.imageDrehen(matrix.getImage(1), matrix.eZeichenDrehen.links), 25, 22, matrix.eTransparent.t, matrix.oled_eFaktor(matrix.eFaktor.f3))
    matrix.writeDigitImageArray(":", 50, 22, 8, 0, matrix.eTransparent.t, matrix.oled_eFaktor(matrix.eFaktor.f4))
    matrix.writeImage(matrix.imageDrehen(matrix.getImage(3), matrix.eZeichenDrehen.halb), 60, 22, matrix.eTransparent.t, matrix.oled_eFaktor(matrix.eFaktor.f3))
    matrix.writeImage(matrix.imageDrehen(matrix.getImage(4), matrix.eZeichenDrehen.xspiegeln), 85, 22, matrix.eTransparent.t, matrix.oled_eFaktor(matrix.eFaktor.f3))
